    ...
    fun createFromFEN(FEN: String): ChessBoard {
        ...
        for (i in 0..7) {
            ...
            for (j in 0 until rows[i].length) {
                if (rows[i][j] == 'p') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.PAWN, PlayerColor.BLACK)
                    colIndex++
                } else if (rows[i][j] == 'r') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.ROOK, PlayerColor.BLACK)
                    colIndex++
                } else if (rows[i][j] == 'n') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.KNIGHT, PlayerColor.BLACK)
                    colIndex++
                } else if (rows[i][j] == 'b') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.BISHOP, PlayerColor.BLACK)
                    colIndex++
                } else if (rows[i][j] == 'q') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.QUEEN, PlayerColor.BLACK)
                    colIndex++
                } else if (rows[i][j] == 'k') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.KING, PlayerColor.BLACK)
                    board.blackKingCol = colIndex
                    board.blackKingRow = 7 - i
                    colIndex++
                } else if (rows[i][j] == 'P') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.PAWN, PlayerColor.WHITE)
                    colIndex++
                } else if (rows[i][j] == 'R') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.ROOK, PlayerColor.WHITE)
                    colIndex++
                } else if (rows[i][j] == 'N') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.KNIGHT, PlayerColor.WHITE)
                    colIndex++
                } else if (rows[i][j] == 'B') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.BISHOP, PlayerColor.WHITE)
                    colIndex++
                } else if (rows[i][j] == 'Q') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.QUEEN, PlayerColor.WHITE)
                    colIndex++
                } else if (rows[i][j] == 'K') {
                    board.fields[colIndex][7 - i] = ChessPiece(PieceType.KING, PlayerColor.WHITE)
        ...
        if (elements[1].compareTo("b") == 0) {
            board.activeColor = PlayerColor.BLACK
        } else {
            board.activeColor = PlayerColor.WHITE
        }
