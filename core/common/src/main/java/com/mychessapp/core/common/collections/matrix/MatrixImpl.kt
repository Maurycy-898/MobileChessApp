...
class MatrixImpl<T>(
  ...
  init {
    if (!isValidMatrix()) throw IllegalMatrixSizeException()
  }
  ...
  override fun transposed(): Matrix<T> = MutableMatrix(rowsSize, columnsSize) { row, column ->
    get(column, row)
  }

  override fun copy(): MatrixImpl<T> = fromLists(rows)
  ...
  companion object {
    ...
    fun <T> fromLists(lists: List<List<T>>) =
      lists
...
class IllegalMatrixSizeException :
  IllegalStateException("In Matrix all rows must have same number of columns")
